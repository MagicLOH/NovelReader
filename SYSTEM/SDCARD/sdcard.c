...
SDIO_D2--  PC10
SDIO_D3 --  PC11            SPI_CS
SDIO_D0 --  PC8             SPI_MISO主机输入
SDIO_D1 --  PC9
SDIO_CMD -- PD2             SPI_MOSI主机输出
SDIO_SCK --  PC12           SPI_SCK时钟
********************************************/
...
/*************SPI读写一字节****************/
static u8 SDcard_WR_Data(u8 data_tx)
{
  u8 data_rx=0;
  u8 i=0;
  for(i=0;i<8;i++)
  {
    SD_SCK=0;//开始发送数据
    if(data_tx&0x80)SD_OUT=1;
    else SD_OUT=0;
    SD_SCK=1;//数据发送完成，开始读取数据
    data_tx<<=1;//继续发送下一位数据
    
    data_rx<<=1;
    if(SD_IN)data_rx|=0x01;
  }
  return data_rx;
}
/******************SD卡发送命令*******************
...
**     cmd_data  --命令参数
**     crc  --校验值
**返回值：0xff --SD卡准备失败，其它值详见命令介绍
**************************************************/
static u8 SDcard_SendCmd(u8 cmd,u32 cmd_data,u8 crc)
{ 
  u16 t=0;
  u8 stat=0;
  /*1.取消片选，结束上一次操作*/
  ...
  do
  {
    stat=SDcard_WR_Data(0xff);
  }while(stat!=0xff && t--);
  if(stat!=0xff)return 0xff;//卡准备失败
  /*3.发送命令*/
  SDcard_WR_Data(cmd|0x40);
  /*4.发送32位命令参数*/
  SDcard_WR_Data(cmd_data>>24);
  SDcard_WR_Data(cmd_data>>16);
  SDcard_WR_Data(cmd_data>>8);
  SDcard_WR_Data(cmd_data&0xff);
  /*5.发CRC校验值*/
  SDcard_WR_Data(crc);
  if(cmd==SDCard_CMD12)SDcard_WR_Data(0xff);
  /*6.等待SD卡响应*/
  t=65535;
  do
  {
...
/*****************SD卡初始化**********************/
TYPE_RES SDcardDevice_Init(void)
{
  u8 i=0;
  u8 t=0;
  u8 stat=0;
  ...
  SDcard_GPIO_Init();
  /*2.发送至少74个时钟脉冲，启动SD卡*/
  for(i=0;i<10;i++)SDcard_WR_Data(0xff);
  /*3.发送CMD0,复位SD卡*/
  t=100;
  do
  {
    stat=SDcard_SendCmd(SDCard_CMD0,0,0X95);
  }while(stat!=0x01 && t--);
  if(stat!=0x01)return TYPE_RESET_ERR;//复位失败
  /*4.发送CMD8,查询SD卡协议版本*/
  stat=SDcard_SendCmd(SDCard_CMD8,0x1AA,0x87);
  if(stat==0x01)//V2.0
  {
    t=100;
    do
    {
      SDcard_SendCmd(SDCard_CMD55,0,0x01);
      stat=SDcard_SendCmd(SDCard_CMD41,0x40000000,0x01);
    }while(stat && t--);
    if(stat)return TYPT_INIT_ERR;//SD初始化失败
    stat=SDcard_SendCmd(SDCard_CMD58,0,0x01);
    if(stat)return TYPT_OCR_ERR;//获取卡信息失败
    stat=SDcard_WR_Data(0xff);
    if(stat&0x40)sd_type=TYPE_SDHC;//V2.0高速卡
    else sd_type=TYPE_STD;//v2.0普通卡
  }
  else//V1.0、MMC
  {
    t=100;
    ...
    if(stat==0)sd_type=TYPE_V1;//V1.0卡
    else
    {
      t=100;
      do
      {
       stat=SDcard_SendCmd(SDCard_CMD1,0,0x01);
      }while(stat && t--);
      if(stat==0)sd_type=TYPE_MMC;//MMC卡
      else sd_type=TYPE_NO;//无法识别
    }
  }
  /*5.取消片选，结束初始化*/
  SD_CS=1;//
...
/***************读取数据包函数*************************/
static u8 SDcard_RecvPack(u8 *buff,u32 cnt)
{
  u32 i=0;
  u16 t=0;
  u8 stat=0;
  ...
  if(stat!=0xfe)return 1;//等待响应失败
  for(i=0;i<cnt;i++)
  {
    buff[i]=SDcard_WR_Data(0xff);
  }
  SDcard_WR_Data(0xff);
  SDcard_WR_Data(0xff);
  return 0;
...
/***********************SD卡读取扇区***********************
**
**形参:u32 sector_addr --扇区地址(从第几个扇区读取数据)
**     buff  --读取数据缓冲区
**     cnt   --读取的扇区数量
**一个扇区512字节
************************************************************/
u8 SDcard_ReadData(u32 sector_addr,u8 *buff,u32 cnt)
{
  if(cnt==1)//读一个扇区
  {
    SDcard_SendCmd(SDCard_CMD17,sector_addr,0x01);
    if(SDcard_RecvPack(buff,512))return 1;//SD卡响应失败
  }
  else//读多个扇区
  {
    SDcard_SendCmd(SDCard_CMD18,sector_addr,0x01);
    while(cnt--)
    {
      if(SDcard_RecvPack(buff,512))return 2;//响应失败
      buff+=512;
    }
    SDcard_SendCmd(SDCard_CMD12,0,0x01);//停止传输
  }
  SD_CS=1;
  SDcard_WR_Data(0xff);
  return 0;  
...
**
*********************************************************/
static u8 SDcard_SendPack(u8 flag,u8 *buff)
{
  u16 i=0;
  u16 t=65535;
  u8 stat=0;
  ...
  if(stat!=0xff)return 1;//SD卡处于忙状态
  SDcard_WR_Data(flag);//发送传输标志
  if(flag!=0xFD)
  {
    for(i=0;i<512;i++)SDcard_WR_Data(buff[i]);
    /*连续发送3个0xff忽略CRC*/
    SDcard_WR_Data(0xFF);
    SDcard_WR_Data(0xFF);
    SDcard_WR_Data(0xFF);
...
/**********************SD卡写扇区***********************
**
**形参:u32 sector_addr --扇区地址(从第几个扇区写入数据)
**     buf  --写入的数据
**     cnt   --写入的扇区数量
**一个扇区512字节
**
*********************************************************/
u8 SDcard_WriteData(u32 sector_addr,u8 *buf,u32 cnt)
{
  if(cnt==1)//写一个扇区
  {
    SDcard_SendCmd(SDCard_CMD24,sector_addr,0x01);
    if(SDcard_SendPack(0xfe,buf))return 1;//SD卡响应失败
  }
  else//写多个扇区
  {
    SDcard_SendCmd(SDCard_CMD55,0,0x01);
    SDcard_SendCmd(SDCard_CMD23,cnt,0x01);//设置写入扇区数量
    SDcard_SendCmd(SDCard_CMD25,sector_addr,0x01);//写入扇区的首地址
    while(cnt--)
    {
      if(SDcard_SendPack(0xfc,buf))return 2;
      buf+=512;
    }
    SDcard_SendPack(0xfd,buf);//结束传输
  }
  SD_CS=1;
  SDcard_WR_Data(0xff); 
  return 0;  
...
u32 SDcard_GetSectorCount(void)
{
  ...
  u8 csd[16]={0};
  /*1.发送CMD9，读取CSD信息*/
  SDcard_SendCmd(SDCard_CMD9,0,0x01);
  SDcard_RecvPack(csd,16);
  SD_CS=1;
  SDcard_WR_Data(0xff);
  if((csd[0]&0xc0)==0x40)
  {
    cszie=csd[9]+(csd[8]<<8)+1;
    cszie<<=10;//得到扇区数量
  }
  return cszie;
}
#else
static u8 CardType=SDIO_STD_CAPACITY_SD_CARD_V2_0;		//SD卡类型（默认为1.x卡）
...
SD_Error SD_Init(void)
{
    ...
    STM32_NVIC_SetPriority(SDIO_IRQn,0,0);
